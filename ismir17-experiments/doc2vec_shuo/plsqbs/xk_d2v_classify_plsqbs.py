...
def build_train_data(sources):
    ...
    data=[]
    labels=[]
    classes=['EHMB','XPKB','XPYAB','XPYB','EHYAB','EHYB','XPMB']
...
def build_test_data(sources,le):
    """build data 2d array using doc2vec repr for classification, and labels, for training and testing"""
    #for each document, loop through the lines, and append the doc2vec representation of each line into the output data file.
    data=[]
    labels=[]
...
sources = {'ehmb_all_test.txt':'TEST_EHMB', 'xpkb_all_test.txt':'TEST_XPKB', 'xpyuanb_all_test.txt':'TEST_XPYB', 'ehyaob_all_test.txt':'TEST_EHYAB', 'xpmb_all_test.txt':'TEST_XPMB', 'ehyuanb_all_test.txt':'TEST_EHYB', 'xpyaob_all_test.txt':'TEST_XPYAB','ehmb_all_train.txt':'TRAIN_EHMB', 'xpkb_all_train.txt':'TRAIN_XPKB', 'xpyuanb_all_train.txt':'TRAIN_XPYB', 'ehyaob_all_train.txt':'TRAIN_EHYAB', 'xpmb_all_train.txt':'TRAIN_XPMB', 'ehyuanb_all_train.txt':'TRAIN_EHYB', 'xpyaob_all_train.txt':'TRAIN_XPYAB'}
...
train_arrays,train_labels,le=build_train_data(sources)
test_arrays,test_labels=build_test_data(sources,le)
